...
static iomux_v3_cfg_t const m2_pads[] = {
		MX7D_PAD_EPDC_PWR_COM__GPIO2_IO30  | MUX_PAD_CTRL(NO_PAD_CTRL), //M.2 Reset#
        MX7D_PAD_SAI2_RX_DATA__GPIO6_IO21  | MUX_PAD_CTRL(NO_PAD_CTRL), //M.2 Power#
};
...
#define M2_POWER_GPIO	IMX_GPIO_NR(6, 21)
#define M2_RST_GPIO		IMX_GPIO_NR(2, 30)
...
int board_init(void)
{
	...
	imx_iomux_v3_setup_multiple_pads(m2_pads, ARRAY_SIZE(m2_pads));
	imx_iomux_v3_setup_multiple_pads(ccm_clko_pads, ARRAY_SIZE(ccm_clko_pads));

	gpio_direction_output(M2_POWER_GPIO, 1);
	gpio_direction_output(M2_RST_GPIO, 1);
	udelay(500);
